...
class DocumentIngestor:
    ...
    def __init__(self, temp_dir:str = "data/multi_doc_chat", faiss_dir: str= "faiss_index", session_id: str | None = None):
        try:
            ...
            self.log.error("Error initializing DocumentIngester:", error = str(e))
            raise DocumentPortalException("Error initializing DocumentIngester", sys)
    ...
    def ingest_files(self, uploaded_files):
        try:
            ...
            for uploaded_file in uploaded_files:
                ...
                self.log.info("Files saved for ingestion", filename=uploaded_file.name, saved_as=str(temp_path), session_id=self.session_id)
            ...
            if not documents:
                raise ValueError("No documents presents to ingest")
            ...
            raise DocumentPortalException("Ingestion error DocumentIngester", sys)
    ...
    def _create_retriever(self, documents):
        try:
            ...
            self.log.info("Documents splits into chunks", total_chunks=len(chunks), session_id=self.session_id)

            embeddings = self.model_loader.load_embeddings()
            vectorstore = FAISS.from_documents(documents=chunks, embedding=embeddings)

            #Save FAISS index in the session folder
            ...
            raise DocumentPortalException("Retrievel error in DocumentIngester", sys)
